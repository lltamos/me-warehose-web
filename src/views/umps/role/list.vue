<template>
    <div>
        ...
        <page-main>
            <el-button type="primary" size="small" icon="el-icon-plus" @click="onCreate">新增角色</el-button>
            ...
            >
                ...
                <el-table-column prop="oName" label="标题" />
...
export default {
    ...
    beforeRouteEnter(to, from, next) {
        ...
        next(vm => {
            if (!vm.$store.state.settings.enableTabbar && !vm.dialogMode) {
                vm.$store.commit('keepAlive/add', 'UmpsRoleList')
            }
        })
    },
    beforeRouteLeave(to, from, next) {
        if (!this.$store.state.settings.enableTabbar && !this.dialogMode) {
            ...
            if (!['XX'].includes(to.name)) {
                ...
                this.$store.commit('keepAlive/remove', 'UmpsRoleList')
            }
        }
        next()
    },
    ...
    methods: {
        getDataList() {
            ...
            this.$api.get('mock/umps/role/list', {
                params
            }).then(res => {
                this.loading = false
        ...
        onDel(row) {
            this.$confirm(`确认删除「${row.title}」吗？`, '确认信息').then(() => {
                this.$api.post('mock/umps/role/delete', {
                    id: row.id
                }).then(() => {
                    this.getDataList()
